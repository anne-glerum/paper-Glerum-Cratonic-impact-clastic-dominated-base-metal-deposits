...
# One wide simulation does not migrate at all, so start_migration = end_migration = 50.
# Set its start_migration to 25 as well.
dataframe.loc[dataframe["start_migration"] > 25, "start_migration"] = 25
...
for m in models:
  ...
  if Path(m).exists():
    path = base + m
    stat_files = sorted(Path(path).glob('*stats_20*'))
  ...
  # One wide simulation does not migrate at all, it needs dots instead of a solid or dashed line.
  # Left-migrating simulations are listed first, they need a dashed line.
  if (column_number == 0 and model_index == 2):
      sns.lineplot(data=df_model_border_faults,x='time',y='value',hue='type',style=True,dashes=[(0.5,0.5)],ax=axs[0,column_number],palette=palette_n_OFM,legend=False)
  elif dy < 0:
  ...
  if Path(ASPECT_statistics_file).exists():
    ...
    if (column_number == 0 and model_index == 2):
      sns.lineplot(data=dataframe_ASPECT,x='time',y='source_area', color=sim_color, style=True, dashes=[(0.5,0.5)], ax=axs[1,column_number],palette=palette_n_OFM,legend=False) 
    elif dy < 0:
