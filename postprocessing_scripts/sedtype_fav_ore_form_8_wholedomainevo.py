...
for m in model_names: 
  ...
  for t in range(0,len(ASPECT_time_steps)):
    ...
    #FastScape_filename=file_name_base+m+'/VTK/Topography'+FastScape_time_steps[t]+'.vtk'
    ...
    # load state
    LoadState('/scratch/usr/bbpanneg/postprocessing/CERI_cratons/10_greyisotherms_velvec_nosource_nohost.pvsm', LoadStateDataFileOptions='Choose File Names',
    ...
    StratigraphyLUTColorBar.WindowLocation = 'Upper Right Corner'

    # Hide asthenosphere (testing only)
    #asthenosphere = FindSource('Threshold_asthenosphere_z0')
    #Hide(asthenosphere, renderView1)

    # Hide mantleL (testing only)
    #mantleL = FindSource('Threshold_mantle_L_z0')
    #Hide(mantleL, renderView1)
    ...
    print("Done changing settings for whole domain")
